...
def demo_encrypt_ascon(message):
    plaintext=message.encode("utf-8")
    variant = "Ascon-128"
    keysize = 16
...
message = "Hi"
encrypted_message, key, n = demo_encrypt_ascon(message)
print("Encrypted Message:", encrypted_message.hex())
print("Key is: ", key.hex())
print("Nonce is: ", n.hex())


def demo_decrypt_ascon(mess, kee, non):
    variant = "Ascon-128"
    keysize = 16
    
    # Choose a cryptographically strong random key and a nonce that never repeats for the same key
    #text = bytes.fromhex(mess)
    #key = bytes.fromhex(kee)
    #n = bytes.fromhex(non)
    associateddata = b"ASCON"
    

    receivedplaintext = ascon_decrypt(kee, non, associateddata, mess, variant)

    return receivedplaintext

mess = encrypted_message.hex()
kee = key.hex()
non = n.hex()
decr = demo_decrypt_ascon(bytes.fromhex(mess), bytes.fromhex(kee), bytes.fromhex(non))
print(decr)